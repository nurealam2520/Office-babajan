      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-primary/10 border-primary/20">
          <CardContent className="py-4 text-center">
            ...
            <p className="text-xl font-bold text-primary">৳{totalAmount.toLocaleString("bn-BD")}</p>
          </CardContent>
        </Card>
          ...
          {collections.map(c => (
            <Card key={c.id}>
              <CardContent className="py-3 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">৳{parseFloat(c.amount).toLocaleString("bn-BD")}</p>
                  {c.description && <p className="text-xs text-muted-foreground">{c.description}</p>}
                </div>
                <p className="text-xs text-muted-foreground">
                  {new Date(c.collection_date).toLocaleDateString("bn-BD")}
                </p>
              </CardContent>
            </Card>
          ))}
